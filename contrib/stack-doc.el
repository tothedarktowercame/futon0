...
(defun stack-doc--stack-doc-for-line (line)
  (cond
   ...
   ((string-prefix-p "  MUSN:" line)
    "MUSN server status. Shows recent activity count, affect transition count, and active agents.")
   ((string-prefix-p "  Affect:" line)
    "Recent affect transitions detected from conversation turns. Tracks emotional signals (joy, anxiety, frustration, etc.) and novel terms that appear in the lookahead window after affect detection.")
   ((string-match "^    [0-9:]+ .* → \\(activation\\|attraction\\|joy\\|fatigue\\|anxiety\\|withdrawal\\|frustration\\|sadness\\|numbness\\|orientation\\|social\\|regulation\\)" line)
    "Affect transition event. Format: TIME ACTOR → TYPE [terms]. The terms in brackets are novel entities that appeared in the 10-minute window after the affect signal was detected.")
   ((string-prefix-p "  ⚠️" line)
    "Warning emitted by Stack HUD checks.")
   ((string-prefix-p "  Services:" line)
    "Local and remote service health. Services are checked by port availability.")
   ((string-prefix-p "    Local:" line)
    "Services running on this machine. Checked by testing if ports are open on localhost.")
   ((string-prefix-p "    Remote:" line)
    "Services on the remote stack, fetched via Transport's /stack/status endpoint.")
   ((and (string-prefix-p "      Services:" line)
         (string-match-p "Futon1" line))
    "Transport (5050): WebSocket relay for inter-agent messaging. Forum WS (5055): Real-time forum WebSocket server. MUSN HTTP (6065): Multi-User Shared Notebook API. IRC (6667): IRC bridge for chat integration. Agency (7070): Agent coordination and dispatch. Futon1 API (8080): Patterns, trails, and workday data.")
   ((string-prefix-p "      Services:" line)
    "Transport (5050): WebSocket relay. Forum WS (5055): Real-time forum. MUSN HTTP (6065): Shared notebook API. IRC (6667): Chat bridge. Agency (7070): Agent dispatch. UI (6060): Web interface.")
   (t nil)))
