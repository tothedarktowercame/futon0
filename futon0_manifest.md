...
7. **Vitality scanner stub** – `scripts/futon0/vitality/scanner.clj` reads `~/code/storage/futon0/vitality/vitality_scanner.json`, counts recent filesystem touches, inspects Tatami exports when configured, and emits `~/code/storage/futon0/vitality/latest_scan.json` (or `../futon3/resources/vitality/latest_scan.json` when run via systemd) for FUTON3 ingestion.
