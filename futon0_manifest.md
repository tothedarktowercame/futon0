...
6. **Zoom R4 ingest** – `scripts/zoom_sync.py --source /media/$USER/ZOOMR4 --dest ~/media/zoomr4` watches for new WAVs, copies them into `~/media/zoomr4`, converts to MP3 (when ffmpeg is available), and records hashes in `data/zoom_sync_index.json`. Friendly names live in `data/zoom_titles.json` (use `--title SHA=Name` to label tracks) and the script writes `TITLES.TXT` back to the recorder on every run. Optional user-level systemd units live in `systemd/zoom-sync.*` so the ingest runs automatically whenever the recorder mounts.
